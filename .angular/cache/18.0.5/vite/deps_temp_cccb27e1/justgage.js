    ...
    (function(root, factory) {
      ...
      const JustGage = function(config) {
        ...
        if (obj.config.relativeGaugeSize === true) {
          ...
          canvasH = getStyle(document.getElementById(obj.config.id), "height").slice(
            0,
            -2
          ) * 1;
        }
        ...
        obj.canvas.customAttributes.ndl = function(value) {
          ...
          let dlb = w / 15;
          let dw = w / 100;
          if (obj.config.pointerOptions.toplength != null && obj.config.pointerOptions.toplength !== void 0)
            dlt = obj.config.pointerOptions.toplength;
          if (obj.config.pointerOptions.bottomlength != null && obj.config.pointerOptions.bottomlength !== void 0)
            dlb = obj.config.pointerOptions.bottomlength;
          if (obj.config.pointerOptions.bottomwidth != null && obj.config.pointerOptions.bottomwidth !== void 0)
        ...
        if (obj.config.donut) {
          obj.level.transform(
            "r" + obj.config.donutStartAngle + ", " + (obj.params.widgetW / 2 + obj.params.dx) + ", " + (obj.params.widgetH / 2 + obj.params.dy)
          );
        }
        ...
        if (obj.config.textRenderer && obj.config.textRenderer(obj.originalValue) !== false) {
          ...
          obj.originalValue = humanFriendlyNumber(
            obj.originalValue,
            obj.config.humanFriendlyDecimal
          ) + obj.config.symbol;
        } else if (obj.config.formatNumber) {
          ...
          obj.originalValue = ((obj.config.max - obj.originalValue) * 1).toFixed(
            obj.config.decimals
          ) + obj.config.symbol;
        } else {
        ...
        if (obj.config.counter === true) {
          const onFrame = function() {
            ...
            if (obj.config.textRenderer && obj.config.textRenderer(Math.floor(currentValue)) !== false) {
              ...
              obj.txtValue.attr(
                "text",
                ((obj.config.max - currentValue) * 1).toFixed(obj.config.decimals) + obj.config.symbol
              );
            } else {
              obj.txtValue.attr(
                "text",
